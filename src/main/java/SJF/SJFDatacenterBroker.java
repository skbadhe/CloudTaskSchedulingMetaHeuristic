...
public class SJFDatacenterBroker extends SimEntity {
	...
	public void submitCloudlets() {
		...
		Map<Integer, Long> cloudletLengthMap = new HashMap<>();

		for (Cloudlet cloudlet : clList) {
			//System.out.println("Cloudlet id=" + cloudlet.getCloudletId() + " length=" + cloudlet.getCloudletLength());
			cloudletLengthMap.put(cloudlet.getCloudletId(), cloudlet.getCloudletLength());
		}

		// Sorting Hashmap for arrival time
		final Map<Integer, Long> sortedByLengthMap = cloudletLengthMap.entrySet().stream()
				.sorted(Map.Entry.comparingByValue())
				.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (e1, e2) -> e1, LinkedHashMap::new));

		Object[] order = sortedByLengthMap.keySet().toArray();
