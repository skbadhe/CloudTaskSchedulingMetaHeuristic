 ...
 */
public class FCFSDatacenterBroker extends SimEntity {
	...
	protected void processResourceCharacteristicsRequest(SimEvent ev) {
		 ...
		 * ": Cloud Resource List received with " + getDatacenterIdsList().size() +
		 * " resource(s)");
		 */
	...
	public void submitCloudlets() {
		...
		Map<Integer, Double> arrivalTimeMap = new HashMap<>();

		for (int i = 0; i < clList.size(); i++) {
			Cloudlet cloudlet = clList.get(i);
			Integer CloudletID = cloudlet.getCloudletId();
			Double arrivalTime = cloudlet.getSubmissionTime();
			arrivalTimeMap.put(CloudletID, arrivalTime);
		}

		// Sorting Hashmap for arrival time
		final Map<Integer, Double> sortedByArrivalTimeMap = arrivalTimeMap.entrySet().stream()
				.sorted(Map.Entry.comparingByValue())
				.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (e1, e2) -> e1, LinkedHashMap::new));

		Object[] order = sortedByArrivalTimeMap.keySet().toArray();

		/*
		 * for (int i=0;i<order.length;i++) { Cloudlet cloudlet = clList.get((int)
		 * order[i]); int idleVM; for(int j=0;j<vm_list.size();j++) { Vm vm =
		 * vm_list.get(j); //vm.
		 * 
		 * if(!vm.isBeingInstantiated()) { idleVM=vm.getId(); }
		 * 
		 * System.out.println("VMID ="+vm.getId()+" Statehistory = "+vm.getStateHistory(
		 * ).toString()); } //break; }
		 */
		int vm_no = 0;
		for (int i = 0; i < order.length; i++) {
			Cloudlet cloudlet = clList.get((int) order[i]);
			Vm vm = vm_list.get(vm_no);
